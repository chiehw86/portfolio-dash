...
SPEC = {
  # 台股:資料源給到最近一場收盤
  "9901.TW": {"bars": [[_tw[0], 2405.0], [_tw[1], 2440.0]], "last": 2440.0, "pc": 2405.0},
  # 港股
  "9701.HK": {"bars": [[_hk[0], 600.0], [_hk[1], 610.0]], "last": 610.0, "pc": 600.0},
  # 日股:盤中時 last 高於最後一根收盤 → 應判定為盤中
  "9801.T":  {"bars": [[_jp[0], 3400.0], [_jp[1], 3467.0]], "last": 3500.0, "pc": 3467.0},
  # 美股:不在亞股重置範圍內
  "XYZ":      {"bars": [[_us[0], 101.0], [_us[1], 103.0]], "last": 103.0, "pc": 101.0},
}
json.dump(SPEC, open('spec.json', 'w'))
pos = [{"name": k, "ticker": k, "kind": "live", "cur": c} for k, c in
       [("9901:TPE", "TWD"), ("9701:HKG", "HKD"), ("9801:TYO", "JPY"), ("XYZ:NASDAQ", "USD")]]
...
print(f'東京現在 {_jst:%H:%M} → 日股{"已" if _jp_open else "尚未"}開盤,日股那檔期望 pending={not _jp_open}')
print()
...
print()
q = chk('9901:TPE', not _tw_done)
if q:
    assert q['change_pct'] is not None, '漲跌幅要保留(不論開盤與否)'
    print('       → 漲跌幅保留(%+.2f%%);沒有新場次時今日金額歸零,漲跌幅照顯示' % q['change_pct'])
chk('9701:HKG', not _hk_done)
chk('9801:TYO', not _jp_open)      # 日股:即時價高於最後一根收盤 → 判定為盤中
chk('XYZ:NASDAQ', False)
